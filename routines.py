...
class kickoff(routine):
    def __init__(self):
        self.jumped = False
    def run(self,agent):
        main_target = agent.ball.location + ((agent.friend_goal.location-agent.ball.location).normalize()*200)
...
def shotFinder(agent,target_start, target_stop=None):
    ...
    for i in range(1,struct.num_slices,10):
        ...
        ratio,shot_vector,offset = shotConeRatio(agent.me,ball,target_start,target_stop)
        ...
        if time_remaining > 0.1:
            if (ball-agent.me.location).magnitude() / time_remaining < 2300:
                if ratio < -2.0:
                    if agent.me.boost > ((ball[2]-200)/20):
                        shots.append(shot(ball,shot_vector,intercept_time,ratio,offset))
                        agent.gui.star(ball,(255,0, 255, 0))
                        agent.gui.line(ball, ball-(shot_vector*1000), (255,0, 255, 0))
                    else:
                        agent.gui.star(ball,(255,255, 0, 0))
                        agent.gui.line(ball, ball-(shot_vector*1000), (255,255, 0, 0))
                elif car_to_ball[2] < 280 and ratio < -0.25:
                    shots.append(shot(ball,shot_vector,intercept_time,ratio,offset))
                    agent.gui.star(ball,(255,0, 255, 0))
                    agent.gui.line(ball, ball-(shot_vector*1000), (255,0, 255, 0))
                else:
                    agent.gui.star(ball,(255,255, 255, 0))
                    agent.gui.line(ball, ball-(shot_vector*1000), (255,255, 255, 0))
            else:
                agent.gui.star(ball,(255,0, 0, 255))
                agent.gui.line(ball, ball-(shot_vector*1000), (255,0, 0, 255))
        else:
            agent.gui.star(ball,(255,128, 0, 255))
            agent.gui.line(ball, ball-(shot_vector*1000), (255,128, 0, 255))               
    return shots
                
class shot(routine):
    def __init__(self,ball,vector,time,ratio,offset,speed = 0):
        self.ball = ball
    ...
    def run(self,agent):
        ...
        self.render(agent)
    def render(self,agent,color=None):
        if color != None:
            agent.gui.star(self.intercept,color)
            agent.gui.line(self.intercept, self.intercept-(self.vector*1000), color)
        else:            
